...
def load_dataloader(data_dir, batch_size=32, max_len=128, model_name='bert-base-uncased'):
    ...
    # Create train/dev split stratified on intents
...
def evaluate_bert_epoch(dataloader, model, tokenizer, lang, USE_WANDB=False):
    """
    Evaluate the BERT model for one epoch
    """
    model.eval()
    all_refs, all_hyps = [], []
...
def start_training(args):
    ...
    if USE_WANDB:
        wandb.init(project="NLU", config=args)
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        # Validation
        slot_dev, intent_dev = evaluate_bert_epoch(dev_loader, model, tokenizer, lang, USE_WANDB)
        f1 = slot_dev['total']['f']
